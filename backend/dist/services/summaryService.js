...
export function getSummary() {
    ...
    const targetPrev = targets
        ...
        .reduce((s, t) => s + t.target, 0) || 0;
    const revenueCurrent = closedWonInQuarter(CURRENT_QUARTER).reduce((s, d) => s + (d.amount ?? 0), 0);
    const revenuePrev = closedWonInQuarter(PREV_QUARTER).reduce((s, d) => s + (d.amount ?? 0), 0);
